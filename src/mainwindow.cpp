...
void MainWindow::on_action_menuFile_Open_triggered()
{
    QStringList fileNames = QFileDialog::getOpenFileNames(this,
        tr("Open DLT/PCAP/MF4 files"), workingDirectory.getDltDirectory(), tr("DLT/PCAP/MF4/TXT files (*.dlt *.DLT *.pcap *.PCAP *.mf4 *.MF4 *.txt);;DLT files (*.dlt *.DLT);;PCAP files (*.pcap *.PCAP);;MF4 files (*.mf4 *.MF4);; Txt files (*.txt)"));
...
bool MainWindow::openDltFile(QStringList fileNames)
{
    ...
    if (QDltOptManager::getInstance()->get_inputmode() == e_inputmode::STREAM){
        ...
        {
            qDebug() << QString::fromUtf8((char*)importfile.msg.headerbuffer,importfile.msg.headersize);
