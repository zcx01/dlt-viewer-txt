...
void QDltImporter::dltIpCFromTxt(QFile &outputfile, QString fileName, QWidget *parent, bool silent)
{
    Q_UNUSED(silent)
    Q_UNUSED(parent)
    ...
    while(!inputTxt.atEnd())
    {
        int percent = inputfile.pos()*100/inputfile.size();
        if(percent>=progressCounter)
        {
            progressCounter += 1;
            emit progress("TXT:",2,percent); // every 1%
            if((percent>0) && ((percent%10)==0))
        ...
        auto lineContent = inputTxt.readLine().toUtf8();
        QByteArray payload;
        ...
        // msg.setPayload(*const_cast<QByteArray*>(lineContent.toUtf8()));
         msg.setPayload(lineContent);


        // // write DLT message
        msg.getMsg(payload,false);
        quint32 sec=0;
        writeDLTMessageToFile(outputfile,payload,0,0,0,sec,sec%1000000000/1000);
    }
